
import { Icon } from '@iconify/react';
import {Link} from 'react-router-dom';

const Icontxt = ({ iconName, displayText, active,targetLink,onClick }) => {
    return ( 
        //for redirecting pages by help of link because link don dont rerender all comp and ,<a>(ancker) tag rerender all tag
        <Link to={targetLink  } >
     

        <div className="flex items-center justify-start cursor-pointer " onClick={onClick}>
            <div className='px-5 py-2'>
                <Icon icon={iconName}  color={active ? "white" :"gray"} fontSize={30}/>
            </div>
            <div  className={`${active ? "text-white" : "text-gray-400"}  text-sm font-semibold hover:text-white`}>{displayText}</div>
        </div>
        </Link>

    )
}

export default Icontxt;